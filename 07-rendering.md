...
2. Отметьте Use Nodes и Backdrop (для просмотра результата)
3. После рендера добавьте ноду Viewer (Shift + A -> Output -> Viewer), чтобы видеть изменения на фоне
...
| Composite       | Выход: финальный результат                       |
...
* Render Layers -> Glare (Fog Glow) -> Color Balance -> [Виньетирование](https://ru.wikipedia.org/wiki/Виньетирование) -> Composite / Viewer
* Для виньетирования: используйте Ellipse Mask + Blur -> Mix (Multiply) или Apha Over

> 💡 Все эффекты применяются после рендера. Для тестов используйте F12, затем переходите в Compositing и настраивайте ноды.

> ⚠️ Не забудьте подключить итоговый результат к Composite, иначе при сохранении он не будет применен!
...
![alt text](assets/07_how_to_use_viewer_composer.webp)