...
### Выделение похожих (Select Similar)

![Выделение по подобию](assets/02_select_similar.png)

* Material - выделить элементы с тем же материалом
* Area - выделить элементы с одинаковой площадью
* Polygon Sides - выделить полигоны с тем же количеством сторон
* Perimeter - выделить элементы с одинаковым периметром
* Normal - выделить элементы с похожим направлением нормали
* Coplanar - выделить элементы, лежащие в одной плоскости
* Flat/Smooth - выделить грани с тем же типом сглаживания
* Freestyle Face Marks - выделить грани с теми же Freestyle-метками
* Face Regions - выделить связанные области граней с аналогичными свойствами
...
Например при выдавливании созданная новая грань остается существовать. Это может быть большой проблемой в будущем. Проверить наличие проблемы можно немного подвинув грани. Другой маркер проблемы - z-fighting (наслоение граней на друг-друга).

Сразу избавиться от проблемы не сложно - просто используйте `Ctrl + Z`. Если проблема была обнаружена поздно, есть несколько вариантов решения:
...
* Все трансформации по-умолчанию опираются на опорную точку (вращение и изменение масштаба)
...
* Установить курсор в нужном месте
  * Можно использовать `Shift + S` / Cursor to Selected и т.п.
...
В Edit Mode можно выделять целиком всю геометрию, которая связана используя `Ctrl + L` (либо `L`)
...
* `Shift + O` - настройки спада
* `Alt + O` - только связанная геометрия
...
Есть отдельная настройка остроты ребер в меню ПКМ в Edit Mode (или в меню Edge)
...
Иногда может потребоваться специально инвертированные нормали. Большинство игровых движков не рисуют обратную сторону граней. Можно такой эффект включить для Blender и использовать для комнат.

![Демонстрация Backface Culling](assets/02_backface_culling_demo.webp)

Backface Culling включается в настройках Viewport. Для рендера в EEVEE нужно так же включить его в настройках материала
...
* Для размещения изображения в нужной проекции, его достаточно развернуть под нужным углом используя Properties или инструмент поворота `R` (набор цифр и выбор оси позволит указать точный угол)
