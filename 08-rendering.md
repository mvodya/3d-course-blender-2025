...
![Render Output](assets/08_render_output.png)
...
![Metadata Settings](assets/08_render_metadata.png)
...
![Light Scheme](assets/08_light_scheme.png)
...
![Demo Scene 3 Lights](assets/08_demo_scene_3_lights.png)

| Key Light                                  | Rim Light                                  | Fill Light                                   |
| ------------------------------------------ | ------------------------------------------ | -------------------------------------------- |
| ![Key Light](assets/08_demo_key_light.png) | ![Rim Light](assets/08_demo_rim_light.png) | ![Fill Light](assets/08_demo_fill_light.png) |

Все источники вместе:

![Key + Rim + Fill](assets/08_demo_all_light.png)
...
- **Color** / **Temperature - Tint** - цвет света
...
> ‼️ Обычно не нужно менять, если нет задачи имитировать реальные камеры

---

#### Clip Start / End (Область видимости, пространство отсечения)
...
| Без DoF                                        | С DoF                                    |
| ---------------------------------------------- | ---------------------------------------- |
| ![Without DoF](assets/08_demo_without_dof.png) | ![With DoF](assets/08_demo_with_dof.png) |
...
Compositing - это финальная обработка изображения после рендера, которая позволяет добавить эффекты, скорректировать цвета и объединить разные рендер-проходы. В Blender для этого используется Compositor, основанный на системе нод (как Shader Editor)

### Активация Compositor

1. Перейдите во вкладку Compositing в верхнем меню (или откройте новое окно с типом "Compositor")
2. Нажимите на New в Compositor

### Основные ноды

| Нода            | Назначение                                       |
| --------------- | ------------------------------------------------ |
| Render Layers   | Вход: изображение с рендера                      |
| Group Output    | Выход: финальный результат                       |
| Viewer          | Предпросмотр результата                          |
| Glare           | Эффекты свечения, бликов (напр. для нити накала) |
| Color Balance   | Цветокоррекция                                   |
| Hue/Saturation  | Яркость, насыщенность                            |
| Lens Distortion | Хроматическая аберрация и искажения линзы        |
| Blur            | Размытие (например, для виньетирования)          |
| Mix             | Смешивание двух изображений                      |
| Alpha Over      | Наложение слоев с альфа-каналом                  |

### Пример базовой схемы

* Render Layers -> Glare (Fog Glow) -> Color Balance -> [Виньетирование](https://ru.wikipedia.org/wiki/Виньетирование) -> Group Output / Viewer
* Для виньетирования: используйте Ellipse Mask + Blur -> Mix (Multiply) или Apha Over (для Blender 5.0+ есть готовая нода)

> 💡 Все эффекты применяются после рендера. Для тестов используйте F12, затем переходите в Compositing и настраивайте ноды.

> ⚠️ Не забудьте подключить итоговый результат к Group Output, иначе при сохранении он не будет применен!

### Быстрое подключение Viewer

Чтобы быстро просматривать результат разных нод, имеет смысл использовать комбинацию: `Ctrl + Shift + ЛКМ`. Клик по ноде автоматически подключит результат ноды к Viewer.

![How to use viewer](assets/08_how_to_use_viewer_composer.webp)
