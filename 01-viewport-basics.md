...
* Окно splash screen (открыть его обратно можно нажав на иконку слева от File)
* Edit / Preferences / Interface
  * Включить в Editors / Status Bar: Screen statistics
...
* Outliner - обход графа сцены
* Properties - правка свойств сцены или активного объекта
* Timeline - временная шкала для управления анимацией и т.п.

Все окна в Blender масштабируется и могут быть перемещены, закрыты, дублированы, изменены.
...
Файл создается в меню File / New. Есть определенные пресеты. Выбирать лучше всего General. Стандартная сцена с кубом, камерой и светом может быть изменена.
...
* Масштабирование (приближение) - Колесо мыши или `Ctrl + СКМ`
* Режим полета - `Shift + ~`
  * Отменить перемещение - `ESC`
...
> ⚠️ В случае отсутствия Gizmo проверить `Ctrl + ~`
...
В редакторе свойств объекта можно задать расположение, поворот, и масштабирование. Помимо базовых свойств, доступно еще множество других полезных настроек: настройка родителя, выбор коллекции, настройки отображения и т.п.

![Редактор свойств объекта](assets/01_object_props.png)

Обратите внимание, что окно Properities позволяет настраивать не только свойства выбранного объекта. Сверху вниз вкладки позволяют по уровням настраивать: параметры инструмента, настройки рендера, настройки вывода рендера, настройки слоя визуализации, настройки сцены, настройки мира. Перечисленное не зависит от выбранного в данный момент объекта. Настройки коллекции зависят от выбранной коллекции. Все вкладки ниже - зависят от выбранного объекта.
...
* `Alt + D` - создание связано копии (не будет виден результат без изменения геометрии объекта, см. второе занятие)
...
В Blender при использовании инструмента если нажать `ESC` - действие будет отменено (произойдет откат). Если нажать `Enter` - изменение зафиксируется. Это справедливо для большинства инструментов в Blender. Во многих случаях (для Move, Scale, Rotate) достаточно нажать `ЛКМ`.
...
![Точная настройка параметров трансформации](assets/01_edit_after_use_tool.webp)
...
В меню Object / Apply / Rotation, Location, Scale - можно выбрать, какие трансформации применить. Применение трансформаций сбросит значения трансформаций объекта в 0 (для Scale - в 1), при этом реальная трансформация изменена не будет. Увидеть изменения можно в Properties объекта.

На самом деле ничего изменяться не будет - точка начала координат объекта (origin) будет сброшена глобальное начало координат, а значения маштаба вернутся в 1. Все трансформации будут применены к вершинам (мешу) объекта.

Данный инструмент очень полезен в ситуациях, когда отмасштабированный объект требуется развернуть или отзеркалить (или у отзеркаленного объекта сбросить отрицательный масштаб). Перед экспортом моделей в игровой движок так же следует применять трансформации во избежании дальнейших сложностей при работе с моделью. Данная функция доступная в виде опции при экспорте.

Delta Transform не влияет на работу инструментов (например локальные вращения будут сохранять ось), но в Properties будут отображены нули.
...
* `X`, `Y`, `Z` - по соответствущим осям после начала применения инструмента
...
* `Shift + X`, `Shift + Y`, `Shift + Z` - по соответствущим плоскостям после начала применения инструмента
...
* Select Circle - выделение окружностью
  ...
  * При нажатии `C` можно временно увеличить радиус (используя колесо мыши)
...
> ⚠️ Лучше не пользоваться без особой необходимости этими инструментами. Стандартные `G`, `R`, `S` вызывать быстрее.
...
* Повторное нажатие на линейку позволяет сделать ломаную линию и измерить угол
...
При переключении вида может применятся Numpad. Хоткеи совпадают с Maya и 3Ds Max. Автор данного курса активно хейтит Numpad и использует TKL клавиатуры, поэтому информация как использовать Numpad в курсе отсутствует.
...
Альтернативный вариант: использование Gizmo в правом верхнем углу (нажатие на X / Y / Z переключит виды)
...
* Переключить вид можно в меню View / Perspective/Orographic
...
* Properties / Object / Relation / Parent
